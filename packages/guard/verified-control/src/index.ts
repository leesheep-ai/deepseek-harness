...
const stateSchema = zod.object({
  ...
  lastTool: zod.object({
    ...
    repeated: zod.number().int().nonnegative(),
  }),
...
export const verifiedControlProjection = {
  ...
  apply: (state: VerifiedControlState, event: SessionEvent) =>
    applyVerifiedControlEvent(state, event),
  stateVersion: 3,
...
export function apply(ctx: Context, input: Config = {}): void {
  ...
  installTransactionRuntime(ctx, {
    mutationTools: policy.mutationTools,
    cleanupTimeoutMs: input.cleanupTimeoutMs ?? 10_000,
  })
}
