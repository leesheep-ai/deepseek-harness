...
function prefixIdsAt(events: readonly SessionEvent[], boundSeq: SessionSeq): string[] {
  ...
  if (boundIndex < 0) throw new Error(`verified-control: bound Fable thinking event ${boundSeq} was not on its historical surface`)
  const ids: string[] = []
...
function latestBinding(agent: Agent): FableThinkingBinding | undefined {
  ...
  for (let index = current.length - 1; index >= 0; index -= 1) {
    ...
    if (seq === undefined) throw new Error(`verified-control: retained Fable thinking message ${message.id} has no session event`)
    const events = eventSnapshotThrough(agent.session, seq)
    const header = foldRequestHeader(events)
    if (header === undefined) throw new Error(`verified-control: Fable thinking message ${message.id} has no durable request header`)
    return {
...
/**
 * Explain why replaying the latest retained Fable 5.1 thinking block would
 * violate its prefix binding, or return undefined when the prefix is intact.
 */
export function fablePrefixMismatchReason(agent: Agent, assembly: PromptAssembly): string | undefined {
  ...
  if (!sameIds(binding.historicalPrefixIds, binding.currentPrefixIds)) {
    return `model-visible messages before Fable thinking block ${binding.messageId} changed after that block was produced`
  }
  const currentSystem = renderPrompt(assembly)
  if ((binding.header.system ?? '') !== currentSystem) {
    return `system prompt changed after Fable thinking block ${binding.messageId} was produced`
  }
  if (!sameTools(binding.header.tools, assembly.tools)) {
    return `tool definitions changed after Fable thinking block ${binding.messageId} was produced`
  }
  return undefined
}
...
/**
 * Fail before provider dispatch when a retained Fable 5.1 thinking block would
 * be replayed under a changed system/tool/message prefix. The guard observes
 * the final request route from downstream request middleware, then independently
 * reassembles the prompt surface because the core request event deliberately
 * exposes configuration rather than mutable message content.
 */
...
/** Cheap preflight used to avoid a second prompt assembly before Fable has emitted bound thinking. */
