...
export function normalizeControlState(raw: Partial<VerifiedControlState>): VerifiedControlState {
  return {
    ...
    delegationContracts: raw.delegationContracts ?? {},
    lastTool: { ...EMPTY_CONTROL_STATE.lastTool, ...(raw.lastTool ?? {}) },
  }
}

function stableToolSignature(name: string, args: string): string { return `${name}:${args}` }
function incident(kind: Incident['kind'], message: string, event: SessionEvent): Incident {
  ...
  return { id: `${kind}:${String(callId ?? event.seq)}:${event.time}`, kind, message, createdAt: event.time, ...(callId === undefined ? {} : { callId: String(callId) }), regressionEval: { name: `regression:${kind}`, assertion: `This trajectory must not reproduce: ${message}` } }
}
...
export function applyVerifiedControlEvent(previous: VerifiedControlState = EMPTY_CONTROL_STATE, event: SessionEvent): VerifiedControlState {
  switch (event.type) {
    ...
    case 'tool/result': {
      ...
      return { ...previous, toolCalls: previous.toolCalls + 1, failures: previous.failures + (isError ? 1 : 0), successfulTools: previous.successfulTools + (isError ? 0 : 1), consecutiveFailures: isError ? previous.consecutiveFailures + 1 : 0, incidents: isError ? [...previous.incidents, incident('tool-failure', event.data.error?.code ?? 'tool call failed', event)] : previous.incidents }
    }
    default: return previous
  }
}

export function validFacts(state: VerifiedControlState, now = Date.now()): Record<string, unknown> { return publicFactValues(state, now) }
