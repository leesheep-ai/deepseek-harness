...
Before controlled mutation, network activity, irreversible work, or delegation, create a normal durable goal and call `control_set_contract`. The contract declares the objective, deterministic success checks, safety invariants, non-goals, requested authority, and finite budgets. Deployment configuration can only reduce those requests. The accepted contract is durably bound to that Goal id and its exact objective; editing the Goal objective makes the existing contract stale until `control_amend_contract` is explicitly approved. `control_amend_contract`, human fact attestation, and external-effect reconciliation enter the existing approval path.
...
Effective authority is `contract request ∩ deployment policy`. Tool-call, failure, duration, repeated-call, and delegation budgets are similarly clamped to the stricter side. Failure budgets are tolerance counts: `maxFailures: 0` permits work while no failure has occurred and fails closed after the first observed failure. `control_get_state` and contract establishment remain available when operational budgets are exhausted so recovery or a new Goal cannot deadlock on stale counters.

A Goal Contract is scoped to one durable Goal id and exact objective. A durable Goal `create` or `clear` resets goal-scoped contract, budget, delegation, repetition, duration, and recovery counters. Trusted facts, incident history, and unresolved transaction/external-effect state survive that reset because they describe world or safety state that may still matter to the next Goal. Amending the same Goal Contract does not reset already-consumed budgets.

Goal completion is fail-closed but is treated as a verified commit operation rather than another unit of operational work. `update_goal(action=complete)` remains available at an exhausted tool/failure/duration boundary, but it is denied while any transaction or external effect remains unresolved, while the contract is stale or bound to another Goal, or until all declared success checks and invariants have deterministic verifier coverage and pass. Configured workspace mutations also verify invariants before execution and again before commit.

Autonomous continuation is fail-closed too. At `agent/turn-stopping`, verified control converts hard no-progress or unsafe conditions — exhausted tool/duration budgets, exhausted failure tolerance, repeated-tool stalls, unresolved rollback failure, or unresolved external-effect review — into the native goal `blocked` phase. The existing `goal-round-driver` therefore remains the only owner of automatic rounds and round accounting; verified control never adds a parallel continuation loop.
...
A static verified-control policy section tells the model to establish a durable goal and Goal Contract before controlled work, record durable observations without treating them as self-verified facts, prepare typed delegation contracts before subagent launches, and stop on unresolved rollback or external-effect review. The model also receives the stable schemas for the `control_*` tools. `control_get_state` exposes the durable contract binding (`contractGoalId`) together with freshness-aware facts and recovery state; the full control state is not injected continuously. If a hard continuation condition is reached, the native goal state becomes `blocked`, so the next autonomous goal round is not scheduled. A Fable 5.1 route additionally receives the model/effort-aware guidance above through the existing runtime-context snapshot mechanism; other models receive no Fable contribution. Prefix-binding validation is harness-side and adds no normal model-facing reminder; only a mismatch becomes an explicit request error and blocked-goal reason.
